...
可穿越平台 = 切 layer 实现双态 + 只保留两个穿越入口(`DropThrough` + 上升速度),其余交给物理。三课最值钱:①**区分站/穿看位置、别看速度方向**;②**层名语义和碰撞矩阵对不上是常态,配置型 bug 无报错最难排**;③**物理仿真是离散步进,高速必开 Continuous**。而「两套方案 → 判定三连演进 → 大幅删减」这条路径本身,比任何单个知识点都更能展示一个完整的工程思考过程:从「想要一个功能」到「想清楚每个部分该由谁负责」。

下一步:墙滑 / 墙跳 / 扒墙(水平射线,与穿越逻辑正交),状态机重构也已排上日程。

## 演示

从下方顶上去能穿过、从上方落能站住的实际效果:
